...
3. Select one of the following operations:
   ...
   - **Add local JSON from Folder** 📄 — creates a JSON file from folders generated by the **Export Snippets to Folder** tool
   - **Import Snippets from local JSON** — loads the latest snippet database from a local JSON file created by the **Add local JSON from Folder** tool
